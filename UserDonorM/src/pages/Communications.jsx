...
import { Mail, Send, Plus, Settings, Clock, CheckCircle, AlertCircle, Trash2, Inbox, ChevronDown, ChevronUp } from 'lucide-react';
...
export function Communications() {
  ...
  const fetchReplies = async (msgId) => {
    if (!token) return;
    try {
      const res = await fetch(`${API_URL}/api/messages/${msgId}/replies`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSendReply = async () => {
    if (!replyBody.trim() || !selectedMessage || !token) return;
    setIsSending(true);
    setSendStatus(null);
    try {
      ...
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8">
          <button
            ...
            className={`${activeTab === 'templates' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center`}
          >
            <Mail className="h-4 w-4 mr-2" />Templates
          </button>
          <button
            onClick={() => setActiveTab('automation')}
            className={`${activeTab === 'automation' ? 'border-primary-500 text-primary-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm flex items-center`}
          >
            <Settings className="h-4 w-4 mr-2" />Automation
          </button>
          <button
          ...
          <Card className="flex flex-col overflow-hidden">
            {!selectedMessage ? (
              <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-3">
                ...
                <p className="text-xs">User messages will appear here</p>
              </div>
            ) : (
              <div className="flex flex-col h-full overflow-hidden">
                <div className="px-6 py-4 border-b flex items-start justify-between flex-shrink-0">
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-gray-500 flex-wrap">
                      ...
                      <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${selectedMessage.status === 'Unread' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>
                        {selectedMessage.status === 'Unread' ? <><Clock className="h-3 w-3 mr-1" />Unread</> : <><CheckCircle className="h-3 w-3 mr-1" />Read</>}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
                  ...
                  {replies.length > 0 && (
                    <div>
                      <button onClick={() => setShowReplies(!showReplies)} className="flex items-center gap-1 text-xs font-semibold text-gray-500 uppercase mb-2 hover:text-gray-700">
                          ...
                          {replies.map((r) => (
                            <div key={r.id} className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3">
                              <div className="flex items-center justify-between mb-1">
                                <span className="text-[10px] font-semibold text-blue-600 uppercase">You replied</span>
                                <span className="text-[10px] text-gray-400">{new Date(r.sent_at).toLocaleString()}</span>
                              </div>
                              <p className="text-sm text-gray-700 whitespace-pre-wrap leading-relaxed">{r.reply_body}</p>
                            </div>
                          ))}
                        </div>
                      )}
                ...
                <div className="px-6 py-4 border-t bg-gray-50 flex-shrink-0">
                  <p className="text-xs font-semibold text-gray-500 uppercase mb-2">Reply to {selectedMessage.name}</p>
                  <textarea
                    className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none bg-white"
                    rows={4}
                    placeholder={`Write your reply to ${selectedMessage.email}...`}
                    value={replyBody}
                    onChange={(e) => { setReplyBody(e.target.value); setSendStatus(null); }}
                  />
                  <div className="flex items-center justify-between mt-2">
                    <div>
                      {sendStatus === 'success' && <span className="flex items-center gap-1 text-xs text-green-600 font-medium"><CheckCircle className="h-3.5 w-3.5" /> Reply sent!</span>}
                      {sendStatus === 'error' && <span className="text-xs text-red-500 font-medium">Failed to send. Try again.</span>}
                    </div>
                    <button
