...
const LOGGED_IN_NAV = [
  ...
  { id: 'donate',      label: 'Donate',       icon: Heart },
  { id: 'my-messages', label: 'My Messages',  icon: MessageSquare },
  { id: 'about',       label: 'About Us',     icon: User },
  { id: 'contact',     label: 'Contact',      icon: Mail },
...
export function HeaderBar({ userName, onToggleSidebar, onNavigate }) {
  ...
  const matchedDonors = q.length >= 1
    ? (donors || []).filter(d =>
        d.sponsor?.toLowerCase().includes(q) || d.project?.toLowerCase().includes(q) || d.type?.toLowerCase().includes(q)
      ).slice(0, 3)
    : []

  const matchedDonations = q.length >= 1
    ? (donations || []).filter(d =>
        d.donor?.toLowerCase().includes(q) || d.campaign?.toLowerCase().includes(q) || d.channel?.toLowerCase().includes(q)
      ).slice(0, 3)
    : []
        ...
        <div className="max-h-96 overflow-y-auto">
          {matchedCampaigns.length > 0 && (
            <div>
              ...
              {matchedCampaigns.map((c) => (
                <button key={c.id} type="button" onClick={() => handleResultClick('campaigns')}
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-3">
                    <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded-full ${c.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'}`}>
                      {c.status}
                    </span>
          ...
          {matchedDonors.length > 0 && (
            <div>
              ...
              {matchedDonors.map((d) => (
                <button key={d.id} type="button" onClick={() => handleResultClick('campaigns')}
                  className="w-full flex items-center justify-between px-4 py-2.5 hover:bg-blue-50 transition-colors text-left group border-b border-gray-50">
                  <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 shrink-0 ml-3">
                    <span className="text-xs font-semibold text-gray-700">₱{Number(d.amount || 0).toLocaleString()}</span>
                    <ChevronRight className="h-3.5 w-3.5 text-gray-300 group-hover:text-blue-400 transition-colors" />
                  </div>
                </button>
              ))}
          ...
          {bellOpen && (
            <div className="absolute right-0 top-10 w-80 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden z-50">
              <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
                ...
                <div className="flex items-center gap-3">
                  {totalNew > 0 && (
                    <button onClick={markAllSeen} className="text-[11px] text-blue-600 hover:underline">Mark all read</button>
                  )}
